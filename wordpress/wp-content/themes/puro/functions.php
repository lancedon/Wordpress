<?php
/*
 * Check if all user was logged in, if not redirect to login page
 * Developed by Leandro on 12/12/2016
 * */

add_action( 'get_header', 'chk_login' );
...
add_action( 'after_setup_theme', '_alert' );
...
function _alert(){
	...
	//check if the current location is the events plugging
	if(strpos($_SERVER['REQUEST_URI'], 'wordpress/?post_type=tribe_events')){
		...
		foreach ($events as $post) {
			...
			//if alert method was Screen
			if ($post->AlertMethod == "Screen") {
				//get today
				$today = date("Y-m-d H:i:s");

				//get the alert date
				$_dtAlert = date('Y-m-d H:i:s', strtotime("- $post->AlertTimeInHours Hours", strtotime($post->_EventStartDate)));

				//if the event has not occurred and we are within the warning date
				if ((strtotime($today) <= strtotime($post->_EventStartDate)) && (strtotime($today) >= strtotime($_dtAlert))) {

					//display the alert
					//echo print_r($post);
					echo "<div class='alert'>";
					echo "<h3>Event Alert</h3>";
					echo "<a href='$post->guid'>$post->post_title<br>";
					echo $post->post_content . "<br>";
					echo $post->_EventStartDate . " - " . $post->_EventEndDate;
					echo "</a>";
					echo "</div>";
